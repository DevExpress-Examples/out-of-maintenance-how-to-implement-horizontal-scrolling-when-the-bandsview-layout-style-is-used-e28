...
Namespace WindowsFormsApplication2
	Partial Public Class HScrollableVGrid
		...
		Private Shared bandsInfoProperty As PropertyInfo = GetType(BandsScrollStrategy).GetProperty("BandsInfo", BindingFlags.NonPublic Or BindingFlags.Instance)
		...
		Private Sub UpdateVGridWidth()
			...
			Dim bansInfoObject As Object
			If bandsInfoProperty IsNot Nothing Then
				bansInfoObject = bandsInfoProperty.GetValue(scrollStrategy, Nothing)
			Else
				bansInfoObject = bandsInfoField.GetValue(scrollStrategy)
			End If
			Dim bandsCount As Integer = CInt(Fix((CType(bansInfoObject, IList)).Count))
		...
		Private Sub vGridControl1_SizeChanged(ByVal sender As Object, ByVal e As EventArgs) Handles vGridControl1.SizeChanged
			If FindForm() IsNot Nothing AndAlso FindForm().IsHandleCreated Then
				BeginInvoke(CType(AddressOf UpdateVGridWidth, Action))
			End If
		End Sub
